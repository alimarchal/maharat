        ...
        try {
            ...
            if (in_array($validated['status'], ['Approve', 'Referred'])) {
                ...
                $referralResponseTask = DB::table('tasks')
                    ...
                    ->where('assigned_to_user_id', $mahratInvoiceApprovalTransaction->assigned_to)
                    ->whereNotNull('assigned_from_user_id')
                    ->where('created_at', '>=', now()->subMinutes(5)) // Created within last 5 minutes
                    ->first();
                
                if ($referralResponseTask) {
                    Log::info('=== SKIPPING MAHARAT INVOICE APPROVAL FLOW - THIS IS A REFERRAL RESPONSE ===', [
                        'invoice_id' => $mahratInvoiceApprovalTransaction->invoice_id,
                    ...
                    DB::commit();
                    return response()->json([
                        'message' => 'Maharat invoice approval transaction updated successfully (referral response)',
                        'data' => new MahratInvoiceApprovalTransactionResource(
