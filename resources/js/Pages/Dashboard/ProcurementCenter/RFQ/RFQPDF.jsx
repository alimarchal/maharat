...
export default function RFQPDF({ rfqId, onGenerated }) {
            ...
            try {
                ...
                if (response.data?.data) {
                    ...
                    if (
                        ...
                        try {
                            const costCenterResponse = await axios.get(
                                `/api/v1/cost-centers/${data.cost_center_id}`
                            );
                    ...
                    if (
                        ...
                        try {
                            const subCostCenterResponse = await axios.get(
                                `/api/v1/cost-centers/${data.sub_cost_center_id}`
                            );
                            if (subCostCenterResponse.data?.data) {
                                data.sub_cost_center =
                                    subCostCenterResponse.data.data;
                            }
                        } catch (e) {
    ...
    const generatePDF = async () => {
        try {
            ...
            autoTable(doc, {
                /* empty config to ensure plugin is loaded */
            });
            ...
            try {
                ...
                const logoWidth = 35; // Increase width slightly
            ...
            doc.text("REQUEST FOR QUOTATION", pageWidth / 2, margin + 15, {
                align: "center",
            });
            ...
            doc.text(
                formatDateForDisplay(rfqData.request_date),
                margin + 25,
                margin + 37
            );
            doc.text(
                formatDateForDisplay(rfqData.closing_date),
                margin + 25,
                margin + 44
            );
            ...
            const fitTextInColumn = (text, maxWidth) => {
                if (doc.getStringUnitWidth(text) * 10 > maxWidth) {
                    let truncated = text;
                    while (
                        doc.getStringUnitWidth(truncated + "...") * 10 >
                        maxWidth
                    ) {
            ...
            doc.text(
                fitTextInColumn(
                    getSafeValue(rfqData, "organization_name"),
                    maxLeftWidth
                ),
                margin + 30,
                margin + 60
            ...
            doc.text(
                fitTextInColumn(
                    getSafeValue(rfqData, "organization_email"),
                    maxLeftWidth
                ),
                margin + 30,
                margin + 67
            ...
            doc.text(
                fitTextInColumn(getSafeValue(rfqData, "city"), maxLeftWidth),
                margin + 30,
                margin + 74
            );
            doc.text(
            ...
            doc.text(
                fitTextInColumn(
                    getSafeValue(rfqData, "contact_number"),
                    maxLeftWidth
                ),
                margin + 30,
                margin + 88
            ...
            doc.text(
                fitTextInColumn(categoryName, maxRightWidth),
                valueX,
                margin + 60
            );
            ...
            doc.text(
                fitTextInColumn(costCenterName, maxRightWidth),
                valueX,
                margin + 67
            );
            ...
            doc.text(
                fitTextInColumn(subCostCenterName, maxRightWidth),
                valueX,
                margin + 74
            );
            ...
            doc.text(
                fitTextInColumn(paymentTypeName, maxRightWidth),
                valueX,
                margin + 81
            );
            ...
            doc.text(
                fitTextInColumn(statusName, maxRightWidth),
                valueX,
                margin + 88
            );
            ...
            const pdfFile = new File(
                [pdfBlob],
                `RFQ_${rfqData.rfq_number || rfqId}.pdf`,
            ...
            const uploadResponse = await axios.post(
                `/api/v1/rfqs/${rfqId}/upload-document`,
                formData,
                {
                    headers: {
