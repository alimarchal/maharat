        ...
        const fetchBudgetRequests = async () => {
            ...
            try {
                const response = await axios.get(
                    `/api/v1/request-budgets?include=fiscalPeriod,department,costCenter,subCostCenter,creator,reallocationHistory.sourceBudgetRequest,reallocationHistory.destinationBudgetRequest&page=${currentPage}&per_page=15&sort=-created_at&filter[type]=budget_request`
                );
                                ...
                                <td className="py-3 px-4">
                                    {(() => {
                                        if (request.type === 'reallocation') {
                                            // For reallocations, use the source budget's previous_year_budget_amount
                                            // This shows the previous year budget of the source sub cost center
                                            const sourcePreviousBudget = request.reallocation_history?.source_budget_request?.previous_year_budget_amount;
                                            if (sourcePreviousBudget !== null && sourcePreviousBudget !== undefined) {
                                                return `${sourcePreviousBudget.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                                            }
                                            // Fallback to source_old_approved_amount from history if source budget not loaded
                                            const historyPreviousBudget = request.reallocation_history?.source_old_approved_amount;
                                            if (historyPreviousBudget !== null && historyPreviousBudget !== undefined) {
                                                return `${historyPreviousBudget.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                                            }
                                            return 'N/A';
                                        }
                                        // For regular budget requests, use previous_year_budget_amount
                                        return request.previous_year_budget_amount 
                                            ? `${request.previous_year_budget_amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                                            : 'N/A';
                                    })()} SAR
                                </td>
                                <td className="py-3 px-4">
                                    {(() => {
                                        if (request.type === 'reallocation') {
                                            ...
                                            if (historyAmount !== null && historyAmount !== undefined && historyAmount !== 0) {
                                                return `${historyAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                                            }
                                            ...
                                            if (requestAmount !== null && requestAmount !== undefined && requestAmount !== 0) {
                                                return `${requestAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                                            }
                                            // If both are 0 or null, show 0 (shouldn't happen if history is loaded)
                                            return '0.00';
                                        }
                                        // For regular budget requests, use requested_amount
