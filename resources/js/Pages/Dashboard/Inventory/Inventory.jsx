...
export default function InventoryTracking({ auth }) {
    ...
    const fetchInventories = async () => {
        ...
        try {
            const response = await axios.get(`/api/v1/inventories?page=${currentPage}`);
        ...
        <AuthenticatedLayout user={auth.user}>
            <div className="min-h-screen p-6">
                ...
                <div className="flex items-center text-[#7D8086] text-lg font-medium space-x-2 mb-6">
                    ...
                    <FontAwesomeIcon icon={faChevronRight} className="text-xl text-[#9B9DA2]" />
                    <span className="text-[#009FDC] text-xl">Inventory Tracking</span>
                </div>
                <Head title="Inventory Tracking" />

                <div className="w-full overflow-hidden">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-[32px] font-bold text-[#2C323C]">Inventory Tracking</h2>
                    </div>
                    ...
                    <div className="w-full overflow-hidden">
                        ...
                        <table className="w-full">
                            <thead className="bg-[#C7E7DE] text-[#2C323C] text-xl font-medium text-left">
                                <tr>
                                    <th className="py-3 px-4 rounded-tl-2xl rounded-bl-2xl text-center">ID</th>
                                ...
                                <tbody className="bg-transparent divide-y divide-gray-200">
                                    {inventories.length > 0 ? (
                                        inventories.map((inventory, index) => (
                                            <tr key={inventory.id}>
                                                <td className="px-6 py-4 text-center">{inventory.id}</td>
                                                <td className="px-6 py-4 text-center">
                                                    {editingId === inventory.id ? (
                                                        ...
                                                        warehouses.find(w => w.id === inventory.warehouse_id)?.name || 'N/A'
                                                    )}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    {editingId === inventory.id ? (
                                                        ...
                                                        products.find(p => p.id === inventory.product_id)?.name || 'N/A'
                                                    )}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    {editingId === inventory.id ? (
                                        ...
                                        <tr>
                                            <td colSpan="7" className="text-center">No inventories available.</td>
                                        </tr>
                                    )}
                                </tbody>
                            )}
